...
```{r worldcancer, echo=FALSE, fig.align='center'}
...
```{r eestiintsidents, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
...
ggplot(kasv_kokku, aes(x = Aasta, y = value, fill = Sugu)) + 
  geom_bar(stat="identity") + 
...
```{r eestipaikmed, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
...
```{r sagedasemadpaikmed, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
kasv %>%
  filter(!grepl("^ ?([[:punct:]]+)|Kõik paikmed", Paige)) %>% 
